// Nouvelle intégration Stripe complètement refaite et adaptée au schéma existant
...
export class StripeServiceNew {
    ...
    try {
      ...
      const sessionParams: Stripe.Checkout.SessionCreateParams = {
        ...
        metadata: {
          ...
          organizationName: currentOrg.name || 'Organisation'
        },
    ...
    try {
      ...
      if (organization.length > 0) {
        ...
        await emailServiceEnhanced.sendSubscriptionConfirmation(
          ...
          organization[0].name || 'Organisation',
          plan.name,
          plan.price,
          expiresAt
        );
  ...
  static async cancelSubscription(organizationId: string): Promise<boolean> {
    try {
      ...
      const stripeSubscriptionId = organization[0].stripeSubscriptionId;
      ...
      const success = await SubscriptionService.cancelSubscription(organizationId);
  ...
  private static async handlePaymentFailed(paymentIntent: Stripe.PaymentIntent) {
    ...
    if (paymentIntent.metadata?.organizationId && paymentIntent.metadata?.organizationEmail) {
      try {
        await emailServiceEnhanced.sendPaymentFailed(
          paymentIntent.metadata.organizationEmail,
          paymentIntent.metadata.organizationName || 'Organisation'
        );
      } catch (error) {
  ...
  private static async handleSubscriptionCanceled(subscription: Stripe.Subscription) {
    ...
    try {
      // Trouver l'organisation par l'ID d'abonnement
      ...
      if (organization.length > 0) {
        await SubscriptionService.cancelSubscription(organization[0].id);
      }
    } catch (error) {
  ...
  static async createOrGetCustomer(email: string, name?: string): Promise<string> {
    try {
      ...
      const customer = await stripe.customers.create({
        email: email,
        name: name || 'Organisation TeamMove'
      });
